...
$res_solicitud=mysql_query($sql_solicitud_inventario);
	...
	if(mysql_num_rows($res_solicitud) > 0){
		echo '<br><b><a href="adm/solicitudes_creadas.php"><font color="red">Solicitud Inventario('.mysql_num_rows($res_solicitud).')</font></a></b>';
		}
		else{
			
		echo '	<br><b><a href="adm/solicitudes_creadas.php"><font color="black">Solicitud Inventario</font></a></b>';
			}
...
do{
 ...
 echo '<form action="precios.php?palabraBusca='.$palabraBusca.'&refBusca='.$refBusca.'&buscar=Buscar" method="post">';
	 
	 if($marca1 == "todo"){ /// si seleccionan todas las marcas  se hace una busqueda en la existencia de todas las marcas
     echo '<input type="hidden" name="marca1" value="'.$marca1.'" />';
	 $count=0;
	 unset($filtro_almacen);
     foreach($cadena as $una_marca ){

	$sql_filtro="SELECT enReserva FROM Precio".$una_marca." WHERE descripcion like '%".$palabraBusca."%' or ref like '%".$palabraBusca."%'";

$resultado_filtro = mysql_query($sql_filtro);
 
while($row_filtro =mysql_fetch_array($resultado_filtro)){
$filtro_almacen[$count]=$row_filtro['enReserva'];	
$count ++;
}
	}
	...
	elseif($marca1 != "todo"){
	echo '<input type="hidden" name="marca1" value="'.$marcaBuscar.'" />';
	$count=0;
	unset($filtro_almacen);
	$sql_filtro="SELECT enReserva FROM Precio".$marcaBuscar." WHERE descripcion like '%".$palabraBusca."%' or ref like '%".$palabraBusca."%'";
...
$filtro_almacen= array_values(array_unique($filtro_almacen));
sort($filtro_almacen);
	
echo '<th> <select name="existencia" id="existencia" onchange="submit()">';
echo "<option value='' select ='selected' >Almacen</option>";

foreach($filtro_almacen as $element ){
...
echo '</form>';
	...
	echo'<td><b>Moneda</b><br><img src="Line.jpg" width="45" height="2" /></td><td><b>Clase</b><br><img src="Line.jpg" width="40" height="2" /></td></tr>';
...
if ($refBusca!='') {
   ...
   if($existencia=='en almacen'){
	   $insertSQL="SELECT * FROM Precio".$marcaBuscar." WHERE ref='".$refBusca."' and enReserva != '0'"; 
	   }
   elseif($existencia != '' and $existencia !='en almacen'){
	  $insertSQL="SELECT * FROM Precio".$marcaBuscar." WHERE ref='".$refBusca."' and enReserva='".$existencia."'"; 
	   }
	   else{
   ...
   if ($row['category']==$cat || $cat=='') {
    ...
    while ($r = mysql_fetch_array($res)) {
		...
		if (strstr($marcaBuscar,$marca)) {
			...
			if($existencia=='en almacen'){
			$result = mysql_query("SELECT * FROM Precio".$r['marca']."  where enReserva !='0' ORDER BY ref");
				}
			
			elseif($existencia != '' and $existencia !='en almacen'){
			$result = mysql_query("SELECT * FROM Precio".$r['marca']."  where enReserva='".$existencia."' ORDER BY ref");
				}
...
$i=0;
echo "<form action='precios.php' method='post' >"; 
