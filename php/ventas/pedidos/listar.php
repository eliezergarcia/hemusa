<?php 
	...
	function noentregado($buscar, $conexion_usuarios){
		...
		$query = "SELECT pedidos.*, contactos.nombreEmpresa FROM pedidos LEFT JOIN contactos ON contactos.id = pedidos.cliente WHERE (cotizacionRef LIKE '%$buscar%' OR numeroPedido LIKE '%$buscar%' OR nombreEmpresa LIKE '%$buscar%' OR contacto LIKE '%$buscar%' OR vendedor LIKE '%$buscar%' OR fecha LIKE '%$buscar%') AND entregado = '0000-00-00' AND fecha >= '2017-01-01' ORDER BY fecha LIMIT 500";
